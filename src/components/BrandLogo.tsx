import markAsset from '@/assets/brand/swing-rights-mark.png';
import lockupAsset from '@/assets/brand/swing-rights-lockup.png';
import { cn } from '@/lib/utils';

export function BrandMark({ className }: { className?: string }) {
  return <img src={markAsset} alt="" aria-hidden className={cn('h-8 w-8 object-contain sm:h-10 sm:w-10', className)} />;
}
...
export function BrandLockup({ className }: { className?: string }) {
  return <img src={lockupAsset} alt="SWING RIGHTS" className={cn('w-[200px] max-w-full object-contain', className)} />;
}