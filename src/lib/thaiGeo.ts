...
export function resolveAreaCoords(
  ...
  // (normalizeGeoText turns ุ into a space, so match the raw string first)
  const np = /^กรุงเทพ/.test(province.trim())
    ? normalizeGeoText('กรุงเทพมหานคร')
    : normalizeGeoText(province);
  if (!np) return null;
