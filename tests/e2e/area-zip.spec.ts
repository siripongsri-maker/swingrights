...
test('AreaPicker searches by postcode at every level and shows zip chips', async ({ page }) => {
  ...
  const provInput = page.getByPlaceholder('ค้นหา จังหวัด... (TH/EN)');
  await provInput.fill('10200');
  ...
  const distInput = page.getByPlaceholder('ค้นหา อำเภอ/เขต... (TH/EN)');
  await distInput.fill('10 200');
  ...
  const tambonInput = page.getByPlaceholder('ค้นหา ตำบล/แขวง... (TH/EN)');
  await tambonInput.fill('10-200');
